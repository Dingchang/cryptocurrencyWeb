    ...
    if length(notifs) != 0 do
      ...
      cond do
        (notif.above and above_threshold) ->
          ...
          CryptoTracker.Mailer.send_alert_email(user.email, "#{currency}'s current price: $#{curr_price} has gone above your $#{threshold}!")
        ...
        (not notif.above and not above_threshold) ->
          ...
          CryptoTracker.Mailer.send_alert_email(user.email, "#{currency}'s current price: $#{curr_price} has gone below $#{threshold}!")
          CryptoTracker.Track.delete_notification(notif)
